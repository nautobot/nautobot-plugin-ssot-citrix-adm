...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SSOT_CITRIX_ADM_xxx
namespace = Collection("nautobot_ssot_citrix_adm")
namespace.configure(
    {
        "nautobot_ssot_citrix_adm": {
            "nautobot_ver": "2.3.1",
            "project_name": "nautobot-ssot-citrix-adm",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_ssot_citrix_adm.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_ssot_citrix_adm.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_ssot_citrix_adm.nautobot_ver,
        "PYTHON_VER": context.nautobot_ssot_citrix_adm.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_ssot_citrix_adm.project_name}",
        f'--project-directory "{context.nautobot_ssot_citrix_adm.compose_dir}"',
    ]

    for compose_file in context.nautobot_ssot_citrix_adm.compose_files:
        compose_file_path = os.path.join(context.nautobot_ssot_citrix_adm.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_ssot_citrix_adm.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_ssot_citrix_adm.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_ssot_citrix_adm.python_ver
    dockerfile_path = os.path.join(context.nautobot_ssot_citrix_adm.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_ssot_citrix_adm.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_ssot_citrix_adm.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_ssot_citrix_adm.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_ssot_citrix_adm"
...
@task(
    help={
        "version": "Version of Nautobot SSoT Citrix ADM to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_ssot_citrix_adm'
    run_command(context, command)
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_ssot_citrix_adm",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_ssot_citrix_adm/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_ssot_citrix_adm.local):
